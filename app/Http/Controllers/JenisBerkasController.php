...
class JenisBerkasController extends Controller
{
    ...
    public function store(Request $request)
    {
         $input = $request->validate([
            'id_berkas' => 'max:11|required|unique:jenis_berkas',
            'jenis_Berkas' => 'max:100|required',
        ]);
     ...
     */
    public function update(Request $request, JenisBerkas $jenisBerkas)
    {
         $input = $request->validate([
            'id_berkas' => ['max:11','required',Rule::unique('jenis_berkas', 'id_berkas')->ignore($jenisBerkas->id)],
            'jenis_Berkas' => 'max:50|required',
        ]);

        $jenisBerkas->update($input);
        return redirect()->route('.index')->with('success', 'Data Jenis Berkas Berhasil Diubah');
    }
